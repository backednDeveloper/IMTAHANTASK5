...
public class Methods {
    ...
    public static String deleteAbsent(String title){
        return Arrays.stream(new String[]{title.trim().replace(" ", " ")}).map(String::toString).collect(Collectors.toList()).toString();
    }
    public static String stringUpdate(List<String> list){
        return stringUpdate(list);
    }
    ...
    public static String arrayDoldurmag(String title){
        return Arrays.stream(title.split(" ")).map(String::toString).collect(Collectors.toList()).toString();
    }
    public static String stringarrayDetected(List<String>list){
         return list.stream()
                 .filter(s-> s.toUpperCase().startsWith("J") && s.toUpperCase().endsWith("A")).collect(Collectors.toList()).toString();
    }
